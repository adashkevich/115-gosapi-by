class DataGrabberJob < ApplicationJob
  ...
  def perform(*args)
    ...
    ActiveRecord::Base.connection.execute 'UPDATE problems AS p SET date_in_progress = cl.change_date FROM change_logs AS cl WHERE p.status = \'4\' AND p.date_in_progress IS NULL AND p.id = cl.problem_id AND cl.id = (SELECT id FROM change_logs  WHERE problem_id = p.id AND field=\'status\' AND new=\'4\' ORDER BY change_date DESC LIMIT 1);'
  end
end
